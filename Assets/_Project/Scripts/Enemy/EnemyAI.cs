...
public abstract class EnemyAI : MonoBehaviour
{
    ...
    protected void HandleStandarBehavior(float dist)
    {
    ...
    {
        if (_currentStamina < _enemyData.StaminaCostRate) return;
        _currentStamina -= _enemyData.StaminaCostRate;
        if (_currentStamina <= 0) _isExhausted = true;
    ...
    protected void RegenerateStamina()
    {
        ...
        _currentStamina += _enemyData.StaminaRegenRate * Time.deltaTime;
        if (_currentStamina >= _enemyData.MaxStamina)
        {
            _currentStamina = _enemyData.MaxStamina;
            _isExhausted = false;
            Debug.Log("Stamina ripristinata");
        }
    }
    #endregion
    #region - Weapon Logic - 
